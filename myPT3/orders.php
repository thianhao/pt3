...
<head>
  ...
  <title>Macintosh App Ordering System : Orders</title>
  <!-- Bootstrap -->
...
<?php if ($_SESSION["user_level"]=="Admin") { ?>
  ...
  <form id="createform" action="orders.php" method="post" class="form-horizontal">
      <div class="form-group">
        <label for="orderid" class="col-sm-3 control-label">Order ID</label>
        <div class="col-sm-9">
        <input name="oid" class="form-control" id="orderid" readonly placeholder="Order ID" value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_num']; ?>" required>
        </div>
        </div>
      <div class="form-group">
          <label for="odate" class="col-sm-3 control-label">Order Date</label>
          <div class="col-sm-9">
          <input name="orderdate" type="date" class="form-control" id="odate" placeholder="" value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>" required>
        </div>
        </div>
         <div class="form-group">
         ...
         <div class="form-group">
          <label for="orderstaff" class="col-sm-3 control-label">Brand</label>
          <div class="col-sm-9">
          <select name="cid" class="form-control" id="ordercustomer" required>
            ...
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $stmt = $conn->prepare("SELECT * FROM tbl_customers_a170997_pt3");
            $stmt->execute();
            $result = $stmt->fetchAll();
          }
          catch(PDOException $e){
  ...
  <div class="row">
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * from tbl_orders_a170997 LIMIT $start_from, $per_page";
        $sql = "SELECT * FROM tbl_orders_a170997, tbl_staffs_a170997_pt3, tbl_customers_a170997_pt3  WHERE ";
        $sql = $sql."tbl_orders_a170997.fld_staff_num = tbl_staffs_a170997_pt3.fld_staff_num and ";
        $sql = $sql."tbl_orders_a170997.fld_customer_num = tbl_customers_a170997_pt3.fld_customer_num";
      ...
      <tr>
        ...
        <td >
          <a href="orders_details.php?oid=<?php echo $readrow['fld_order_num']; ?>" class="btn btn-warning btn-xs" role="button" id="detail"> Details </a>
          <?php if ($_SESSION["user_level"]=="Admin") {?>
          <a href="orders.php?edit=<?php echo $readrow['fld_order_num']; ?>" class="btn btn-success btn-xs" role="button" id="edit"> Edit </a>
          <a href="orders.php?delete=<?php echo $readrow['fld_order_num']; ?>" onclick="return confirm('Are you sure to delete?');" class="btn btn-danger btn-xs" role="button" id="delete">Delete</a> <?php } ?>
        </td>
      </tr>
      <?php } ?>
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a170997");
            $stmt->execute();
            $result = $stmt->fetchAll();
